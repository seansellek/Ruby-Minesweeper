class Game
  ...
  def score
    ...
    { score: score, good_flags: good_flags, bad_flags: bad_flags, won?: won }
  end
  ...
  def validate_turn(input)
    if input =~ /^[A-#{(board.width - 1 + 65).chr}][1-#{board.width}]$/i
      lett = input.split('')[0]
    ...
    elsif input =~ /^flag [A-#{(board.width - 1 + 65).chr}][1-#{board.width}]$/i
      input = input.split(' ')
